...
class Request extends Component {
        ...
        firestore().collection('Users').get().then(snapshot => {
            ...
            snapshot.forEach(user => {
                if (user.data().userType === 'donor' && user.data().availability) {
                    const distance = this.calculateDistance(this.state.region.latitude, this.state.region.longitude, user.data().lat, user.data().lng);
                    donors.push({
                        id: user.id,
                        lat: user.data().lat,
                        lng: user.data().lng,
    ...
    requestChecker() {
        firestore().collection('Requests').doc(this.state.requestId).onSnapshot(snapshot => {
            if (snapshot.data().status === "accepted") {
                firestore().collection('Users').doc(snapshot.data().donorId).get().then(donor => {
                    this.setState({
                        loading: false,
                        donor: {
    ...
    submitRatings() {
        firestore().collection('Users').doc(this.state.donor.id)
            .get()
            .then((querySnapshot) => {
                ...
                >
                    <View>
                        <View style={{ paddingHorizontal: 10 }}>
                            {(!submitRatings) ? <View style={{ flexDirection: "row", paddingLeft: 10, width: '100%' }}>
                                <Image source={(donor.image === '') ?
                                    require('../../assets/img/user.jpg') :
                                    { uri: 'data:image/jpeg;base64,' + donor.image }} style={imageStyle} />
                                <View style={{ marginLeft: 20, paddingTop: 10 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                                        <EntypoIcon
                                            ...
                                            color={"#ff5d5b"}
                                        />
                                        <Text style={{ color: '#ff5d5b', fontSize: 14 }}>{donor.bloodGroup}</Text>
                                    </View>
                                    <Text style={{ color: '#ff5d5b', fontSize: 14 }}>Address: {donor.address}, {donor.city}</Text>                                    
                                </View>
                            </View> :
                        ...
                        {(!submitRatings) ? <View style={{ flexDirection: "row", justifyContent: "space-evenly", width: '100%', marginBottom: 10 }}>
                            <Button
                                icon="phone"
                                mode="contained"
                                theme={buttonTheme}
