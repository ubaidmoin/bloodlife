...
class Donation extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            options: [
                { type: 'Bank Account', bank: 'Askari Bank', title: 'Ubaid Ullah', accNo: '4324213456643' },
                { type: 'Easy Paisa', bank: 'Easy Paisa', title: 'Ubaid Ullah', accNo: '03325320328' }
            ],
            type: '',
            bank: '',
            title: '',
    ...
    componentDidMount() {
        firestore().collection('DonationOptions').onSnapshot(snapshot => {
            let options = [];
        ...
        return (
            <View style={event} key={index}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Text style={listTextStyle}>{(item.title === "Bank Account") ? 'Account Title:' : 'CNIC:'}</Text>
    ...
    render() {
        const { container, buttonStyle, iconButtonStyle, modalButtonStyle, modalStyle, textStyle } = styles;
        ...
        return (
            <View style={container}>
                <View style={{ width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20 }}>
                    <Text style={{ fontWeight: 'bold', fontSize: 25, color: '#ff5d5b' }}>Donation Options</Text>
                    <TouchableOpacity
                ...
                >
                    <View style={{ flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'center', marginTop: 30 }}>
                        ...
                        <TextInput
                            label={(this.state.selectedIndex === 0) ? 'Bank Name' : 'Account Title'}
                            mode='outlined'
                            style={{
                                height: 40,
                                width: '80%',
...
const styles = StyleSheet.create({
    ...
    modalStyle: {
        borderRadius: 10,
        marginVertical: Dimensions.get('screen').height * .02,
        width: '100%',
        marginLeft: 0,
...
export default connect(userDetailsAction.mapStateToProps, userDetailsAction.mapDispatchToProps)(Donation);